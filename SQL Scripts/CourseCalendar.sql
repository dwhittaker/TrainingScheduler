...
select (select TrainingCourseID from TrainingCourse where CourseTitle =(select RTrim(Ltrim(Course_Title)) from SPIN_DANIC.training_scheduler.dbo.training_course where course_code = cc.Course_Code)),         --RTrim(Ltrim(cc.Course_title)))
coursedescription = 
Case
...
from SPIN_DANIC.training_scheduler.dbo.Course_Calendar cc

update CourseInstance set displayincalendar = 1
--
